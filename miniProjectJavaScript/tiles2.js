...
var levels = [], currentLevel = 0, tileSize = 48;
...
function preload() {
    stage = new createjs.Stage("canvasLevel2");
...
function removeThirdDialog() {
    var removeThirdDialog = document.getElementById('thirdDialog');
    removeThirdDialog.parentNode.removeChild(removeThirdDialog);

    var addCanvas = document.getElementById('canvasLevel2').style.display='block';
    bgMusic.volume = 1;
    return false;
}
    ...
    switch(grid[row][col].tileNumber){
        ...
        case 15, 16, 17, 33, 34, 35, 51, 52, 53, 54,55,56,72,73,74,90,91,92,62,63,64,80,81,82:
         ...
         case 57:
            ...
            break;
        case 60:
            var removeCanvas = document.getElementById("canvasLevel2").style.display='none';
            playVid();
            grid[row][col].gotoAndStop("grass");
            grid[row][col].tileNumber=240;
            return true;
...
var test = new storedWeapons(weapons.pistol, weapons.rifle, weapons.shotgun);
                ...
                window.location = "../FightingLevel2/shooter.html";
        ...
        case 94:
            ...
            if(bonusComplete===1){
              alert('You have already finished the level')
          }else{
                localStorage.setItem('heroLastPosRow', hero.row);
                localStorage.setItem('heroLastPosCol', hero.col);
                localStorage.setItem('coins', coins);
                window.location="../FlappyBirdFINAL/flappy.html";
          }
